...
import { AuthData, LoginValues, SetEmailValues, SignupValues } from "types/auth";
import { SuccessDataResponse, SuccessResponse } from "types/responses";
import { apiErrorParser, commonSuccessRespFilter } from "helpers/responseHelpers";
...
export default class AuthService {
  ...
  private static config = {
    baseURL: process.env.NODE_ENV === 'production'
      ? 'https://anonly.herokuapp.com/auth'
  ...
  static login(loginDetails: LoginValues) {
    return this.http.post('/login', loginDetails)
  ...
  static signup(signupDetails: SignupValues) {
    return this.http.post('/signup', _.pick(signupDetails, ['username', 'password']))
      .then((response: AxiosResponse<SuccessDataResponse<AuthData>>) =>
        commonSuccessRespFilter(response)
      )
      .catch(apiErrorParser);
  ...
  static setEmail(emailDetails: SetEmailValues) {
    return this.httpWithAuthHeader.post('/verification/request', emailDetails)
